...
Future<void> fetch_products() async{
    await FirebaseFirestore.instance.collection('products').get().then((QuerySnapshot productsnap){
      _productsList.clear();
      productsnap.docs.forEach((element) {
        print(element.get('id').runtimeType);
        _productsList.insert(0, ProductModel(title: element.get('title'), id: element.get('id'),
            productCategoryName: element.get('productCategoryName'), imageUrl: element.get('imageUrl'),
            price: double.parse(element.get('price')), salePrice: (element.get('saleprice')+0.0), isOnSale: element.get('isOnSale'),
            isPiece: element.get('isPeiece')));
      });

    });
notifyListeners();
}
  List<ProductModel> get getOnSale {
  ...
  List<ProductModel> findCategoryName(String categoryName) {
    return _productsList
        .where((element) =>
